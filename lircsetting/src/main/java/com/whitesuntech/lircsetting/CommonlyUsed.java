...
public class CommonlyUsed {
	...
	static public String createCSVstringFromList(ListCustom LC, String separator)
	{
		String retString = "";
		for(int i=0;i<LC.size();i++)
		{
			if(i>0) {retString += separator;}
			retString += LC.getVectorElement(i);
		}
	...
	static public String getCurrentDateString(String sep)
	{
	 ...
	 public static boolean[] convertListToBooleanArray(List<Boolean> inputlist)
	 {
		 boolean[] retBoolList = new boolean[inputlist.size()];
		 if (inputlist.size() > 0) {
	            for (int loop = 0; loop < inputlist.size(); loop++) {
	            	retBoolList[loop] = inputlist.get(loop);
	            }
