...
import static aido.properties.PermittedApps.PROP_PERMITTEDAPPS_HASH;
...
public class AppsLoader extends AsyncTaskLoader<ArrayList<AppModel>> {
    ...
    public ArrayList<AppModel> loadInBackground() {
        ...
        for (int i = 0; i < apps.size(); i++) {
            ...
            if(!PROP_PERMITTEDAPPS_HASH.containsKey(apps.get(i).packageName))
            {
                continue;
            }

          // String value= PROP_PERMITTEDAPPS_HASH.get("com.example.archi.health");
//            if(value.equals("Report Something")){
//                Log.i("aaaaaaaaaaaaa","true");
//                Intent i1 = new Intent();
//                i1.setAction("com.company.package.FOO");
//                context.startActivity(i1);
//
//            }
//            else{
//                Log.i("aaaaaaaaaaaaa","false");
//            }

            // only apps which are launchable
            if (context.getPackageManager().getLaunchIntentForPackage(pkg) != null) {
               Log.i("aaaaaaaaaaaaa","caleed");
