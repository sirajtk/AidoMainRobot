...
public class DemoPlay {
    ...
    public DemoPlay(Context context, String PI_IP, TextureView videoview, SurfaceTexture st, String demofile,BroadcastTTS ttsaido, int priority, int behaveid) {
        ...
        _motor = new MotorMessage(_maincontext,PI_IP);
            ...
            else
            {
                if(movname.equalsIgnoreCase("traffic"))
                {
                    ...
                    if(!_motor.isRunning())
                    {
                       ...
                       // _motor.run(panval,tiltval,speed);
                        if(!panval.equals("0") && !tiltval.equals("0")) {
                            _motor.runPi(panval, tiltval);
                        }
                    }
   ...
   public  void interrupt()
    {
        ...
        Log.i("BEHAVE","Interrupted !!! ");
        interrupted = true;
        try {

                mp.stop();
                mp.release();
                System.out.println("MEdia player:released");
                isplaying = false;
