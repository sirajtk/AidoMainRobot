 ...
 * (Note: if you name the license file something else you will need to update the licensePath in the initializeCameraDetector() method in MainActivityAffectiva)
 * - Build the project
 * - Run the app on an Android device with a front-facing camera
 */

public class MainActivityAffectiva extends Fragment
        implements Detector.FaceListener, Detector.ImageListener, CameraDetector.CameraEventListener,
        View.OnTouchListener, DrawingView.DrawingThreadEventListener {
    ...
    private boolean cameraPermissionsAvailable = true;
    private boolean storagePermissionsAvailable = true;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        return inflater.inflate(R.layout.activity_affectiva, container, false);
    ...
    public interface OnDataAvailable {
        void onDataAvailable (FaceDataModel facedata);
    }

    OnDataAvailable dataAvailable;

    public void setOnDataAvailable(OnDataAvailable datalistener) {
    ...
    public void onAttach(Activity activity) {
       ...
       // dataAvailable = (OnDataAvailable) activity;
    ...
    void determineCameraAvailability() {
        ...
        if (!isFrontFacingCameraDetected && !isBackFacingCameraDetected) {
            ...
            TextView notFoundTextView = getView().findViewById(R.id.not_found_textview);
    ...
    void initializeUI() {
        ...
        ViewGroup activityLayout = getView().findViewById(android.R.id.content);
        progressBarLayout = getView().findViewById(R.id.progress_bar_cover);
        permissionsUnavailableLayout = getView().findViewById(R.id.permissionsUnavialableLayout);
        metricViewLayout = getView().findViewById(R.id.metric_view_group);
        leftMetricsLayout = getView().findViewById(R.id.left_metrics);
        rightMetricsLayout = getView().findViewById(R.id.right_metrics);
        mainLayout = getView().findViewById(R.id.main_layout);
        fpsPct = getView().findViewById(R.id.fps_value);
        fpsName = getView().findViewById(R.id.fps_name);
        cameraView = getView().findViewById(R.id.camera_preview);
        drawingView = getView().findViewById(R.id.drawing_view);
        settingsButton = getView().findViewById(R.id.settings_button);
        cameraButton = getView().findViewById(R.id.camera_button);
        screenshotButton = getView().findViewById(R.id.screenshot_button);
        progressBar = getView().findViewById(R.id.progress_bar);
        pleaseWaitTextView = getView().findViewById(R.id.please_wait_textview);
        Button retryPermissionsButton = getView().findViewById(R.id.retryPermissionsButton);

        //Initialize views to display metrics
        metricNames = new TextView[NUM_METRICS_DISPLAYED];
        metricNames[0] = getView().findViewById(R.id.metric_name_0);
        metricNames[1] = getView().findViewById(R.id.metric_name_1);
        metricNames[2] = getView().findViewById(R.id.metric_name_2);
        metricNames[3] = getView().findViewById(R.id.metric_name_3);
        metricNames[4] = getView().findViewById(R.id.metric_name_4);
        metricNames[5] = getView().findViewById(R.id.metric_name_5);
        metricDisplays = new MetricDisplay[NUM_METRICS_DISPLAYED];
        metricDisplays[0] = getView().findViewById(R.id.metric_pct_0);
        metricDisplays[1] = getView().findViewById(R.id.metric_pct_1);
        metricDisplays[2] = getView().findViewById(R.id.metric_pct_2);
        metricDisplays[3] = getView().findViewById(R.id.metric_pct_3);
        metricDisplays[4] = getView().findViewById(R.id.metric_pct_4);
        metricDisplays[5] = getView().findViewById(R.id.metric_pct_5);

        //Load Application Font and set UI Elements to use it
        Typeface face = Typeface.createFromAsset(thiscontext.getAssets(), "fonts/Square.ttf");
    ...
    void initializeCameraDetector() {
         ...
         */
        detector = new CameraDetector(thiscontext, cameraType, cameraView, (multiFaceModeEnabled ? MAX_SUPPORTED_FACES : 1), Detector.FaceDetectorMode.LARGE_FACES);
    ...
    public void mainWindowResumedTasks() {
    ...
    public void onImageResults(List<Face> faces, Frame image, float timeStamp) {
        ...
        if (faces.size() <= 0) {
            ...
            for (MetricDisplay metricDisplay : metricDisplays) {
                ...
                try {
                    ...
                    if(metricDisplay.getMetricToDisplay().getType() == MetricsManager.MetricType.Emotion) {
                        float score = (Float) metricDisplay.getFaceScoreMethod().invoke(faces.get(0).emotions);

                        displaystring += MetricsManager.getUpperCaseName(metricDisplay.getMetricToDisplay()) + "=" + score + "\n";
                    }
            ...
            //faces.get(0).getFacePoints().;


            FaceDataModel facedata = new FaceDataModel(metricDisplays,faces.get(0));


            dataAvailable.onDataAvailable(facedata);


            //CommonlyUsed.showMsg(thiscontext,"GOT : " + displaystring);
    ...
    private void processScreenshot(Bitmap drawingViewBitmap, boolean alsoSaveRaw) {
        if (mostRecentFrame == null) {
            Toast.makeText(thiscontext, "No frame detected, aborting screenshot", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        ImageHelper.addPngToGallery(thiscontext, screenshotFile);
        ...
        if (alsoSaveRaw) {
            ...
            ImageHelper.addPngToGallery(thiscontext, rawScreenshotFile);
        }
        ...
        Toast.makeText(thiscontext, fileSavedMessage, Toast.LENGTH_SHORT).show();
    ...
    private void setCameraType(CameraDetector.CameraType type) {
        SharedPreferences.Editor preferencesEditor = PreferenceManager.getDefaultSharedPreferences(thiscontext).edit();
            ...
            switch (type) {
                case CAMERA_BACK:
                    if (isBackFacingCameraDetected) {
                        ...
                        Toast.makeText(thiscontext, "No back-facing camera found", Toast.LENGTH_LONG).show();
                        return;
                    }
                    break;
                case CAMERA_FRONT:
                    if (isFrontFacingCameraDetected) {
                        ...
                        Toast.makeText(thiscontext, "No front-facing camera found", Toast.LENGTH_LONG).show();
                        return;
                    }
                    break;
                default:
